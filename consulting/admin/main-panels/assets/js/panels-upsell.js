(function() {
    ...
    const injectUpgradeButton = () => {
        ...
        if (!sidebarHeader || document.querySelector('.thinkup-panel-upgrade')) return;
		...
		// Assign element tags
        container.id        = 'thinkup-panel-upgrade';
        container.className = 'thinkup-panel-upgrade';
    ...
    const observer = new MutationObserver(injectUpgradeButton);

	// Observe all new elements in the editor
    observer.observe(document.body, { childList: true, subtree: true });

	// Run once in case the panel is already present
    injectUpgradeButton(); // Run once immediately
